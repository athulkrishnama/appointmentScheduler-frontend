...
import BestServiceProviderChart from "./BestServiceProviderChart";

const Dashboard = () => {
  const [dashboardData, setDashboardData] = useState(null);
  const [loading, setLoading] = useState(true);
  const [appointmentPeriod, setAppointmentPeriod] = useState("weekly");
  ...
  return (
    <div className="w-full h-screen bg-white p-8 overflow-y-auto noScrollBar">
      <div className="max-w-7xl mx-auto">
        <h1 className="text-4xl font-black text-gray-900 mb-10 text-center sticky top-0 backdrop-blur-md py-5">
          Dashboard Overview
        </h1>
              ...
              <div className="flex items-center gap-4">
                ...
                <div>
                  ...
                  <p className="text-3xl font-bold text-gray-900">
                    {dashboardData.activeServiceProviders}
                  </p>
                </div>
              </div>
        ...
        <motion.div className="mt-16">
          ...
          className="px-3 py-2 rounded-lg focus-visible:outline-none appearance-none"
          >
            <option value="weekly">Weekly</option>
            <option value="monthly">Monthly</option>
            <option value="yearly ">Yearly</option>
          </select>
          <AppointmentChart period={appointmentPeriod}/>
        </motion.div>

        <motion.div className="mt-16">
          <BestServiceProviderChart/>
        </motion.div>
      </div>
    </div>
  );
