...
function Header() {
  ...
  return (
    <motion.header 
      className="bg-white text-black p-4 shadow-md sticky top-0 left-0 right-0 z-50 w-full"
                ...
                {navigationLinks.map((link, index) => (
                  <li key={index}>
                    <Link 
                      ...
                      className={`block px-4 py-2 rounded-lg transition-all duration-300 text-lg font-medium ${
                        isActive(link.path) 
                          ? 'bg-gray-900 text-white shadow-lg' 
                          : 'text-gray-800 hover:bg-gray-100'
                      }`}
